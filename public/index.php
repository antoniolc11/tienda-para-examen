...
<body>
    ...
    $where = '';
    $pdo = conectar();
    $sent1 = $pdo->query("SELECT * FROM categorias");

    $cat = obtener_get('categoria');
    $cat != null ? $where = "WHERE categoria_id = $cat" : null;
    ...
    $id = obtener_get('quitar_carrito');

                                        
   if ((obtener_get('quitar_carrito'))) {
        $carrito->borrarLinea($id, $carrito);
    } 
        ...
        <form action="" method="get">
            ...
            <select name="categoria" id="categoria">
                ...
                <?php foreach ($sent1 as $fila) : ?>
                    <option name="categoria" <?= $cat != $fila['id'] ? null : 'selected' ?> value="<?= hh($fila['id']) ?>"><?= hh($fila['nombre']) ?></option>
                <?php endforeach ?>
            </select>
                ...
                <?php foreach ($sent as $fila) : ?>
                    <?php if ($fila['stock'] > 0) : ?>
                        <div class="p-6 max-w-xs min-w-full bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                            <a href="/articulo_show.php?id=<?= $fila['id'] ?>">
                            ...
                            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">Categoria: <?= hh($fila['nombre']) ?></p>
                            <?php if ($fila['descuento'] != null) { ?>
                                <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">Descuento: <?= hh($fila['descuento']) ?>%</p>
                            <?php } ?>
                            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">Precio: <?= dinero(hh($fila['precio'])) ?></p>
                            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">Existencias: <?= hh($fila['stock']) ?></p>
                            <?php if ($fila['stock'] > 0) : ?>
                                <a href="/insertar_en_carrito.php?id=<?= $fila['id'] ?>&categoria=<?= $cat ?>" class="inline-flex items-center py-2 px-3.5 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                    Añadir al carrito
                        ...
                        <table class="mx-auto text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($carrito->getLineas() as $id => $linea) : ?>
                                    ...
                                    $cate_arti = $articulo->getCategoria();
                                    $cantidad = $linea->getCantidad();
                                    $sent2 = $pdo->query("SELECT nombre FROM categorias WHERE id=$cate_arti");
                                    ...
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="py-4 px-6"><?= $cantidad ?></td>
                                        <td class="py-4 px-6 text-center"><?= $sent2->fetch()['nombre'] ?></td>
                                        <td>
                                            <form action="" method="get">
                                                <button class="w-5 h-5" type="submit" name="quitar_carrito" value="<?= $articulo->getId() ?>"> <img src="/img/papelera.png"></button>
                                            </form>
